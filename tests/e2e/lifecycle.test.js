...
describe("missing password", () => {
  ...
  it("running init without WALLET_PASSWORD -> error", () => {
    ...
    assert.ok(
      out.includes("WALLET_PASSWORD is required"),
...
describe("setup.sh idempotency", () => {
  ...
  it("running setup.sh twice does not overwrite existing chains.json", () => {
    ctx = createTestEnv()
    ...
    // Read the config generated by first run (createTestEnv already wrote chains.json, setup.sh should not overwrite)
    ...
    assert.equal(contentAfter, contentBefore, "chains.json should not be overwritten")
